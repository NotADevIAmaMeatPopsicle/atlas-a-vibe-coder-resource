...
import { renderTriageReport, renderTriageReportV1_2_0 } from '../src/triage-report.js';
...
test('current triage Markdown visibly encodes terminal and bidi controls while preserving v1.2 verification', () => {
  const hostile = finding('f', 'high', 'src/unsafe\u001b[31m\u202Ename.ts', 'production');
  hostile.title = 'unsafe\u0007title\u2066';
  hostile.description = 'left\u009Bright';
  hostile.nextValidation = 'inspect\u061cvalue';

  const current = renderTriageReport(RUN, [hostile], []);
  assert.doesNotMatch(current, /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f-\u009f\u061c\u200e\u200f\u2028\u2029\u202a-\u202e\u2066-\u2069]/u);
  assert(current.includes('unsafe\\\\u0007title\\\\u2066'));
  assert(current.includes('src/unsafe\\u001B[31m\\u202Ename.ts'));

  const previous = renderTriageReportV1_2_0(RUN, [hostile], []);
  assert(previous.includes('\u001b'));
  assert(previous.includes('\u202E'));
});

test('current triage control encoding cannot collide with literal escape text', () => {
  const controlled = finding('g', 'medium', 'src/control.ts', 'production');
  controlled.signals = ['x\u001by'];
  const literal = finding('h', 'medium', 'src/literal.ts', 'production');
  literal.signals = ['x\\u001By'];

  const markdown = renderTriageReport(RUN, [controlled, literal], []);
  assert(markdown.includes('`x\\u001By`'));
  assert(markdown.includes('`x\\\\u001By`'));
});
