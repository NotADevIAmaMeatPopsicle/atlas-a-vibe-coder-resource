...
import { cp, mkdir, mkdtemp, open, readFile, rename, rm, stat, symlink, writeFile } from 'node:fs/promises';
...
import { MAX_REVIEW_CAMPAIGN_BYTES, MAX_REVIEW_PACKET_BYTES } from '../src/reviews.js';
import { MAX_VERIFIER_ARTIFACT_BYTES } from '../src/security/bounded-artifacts.js';
...
test('review status bounds campaign and packet JSON before parsing', async (context) => {
  const maximumMultibytePurposeBytes = Buffer.byteLength('界'.repeat(4096)) * 25_000;
  assert(MAX_REVIEW_CAMPAIGN_BYTES > maximumMultibytePurposeBytes + MAX_VERIFIER_ARTIFACT_BYTES);
  const fixture = await createFixture();
  context.after(async () => rm(fixture.root, { recursive: true, force: true }));
  ...
  const review = await createReviewCampaign({
    ...
    workspacePath: path.join(fixture.root, 'workspace'),
    targetConfigPath: fixture.targetConfig,
    purpose: 'Bounded review fixture',
    selection: 'all',
    batchSize: 2
  });
  const campaignPath = path.join(review.directory, 'campaign.json');
  const campaignFile = await open(campaignPath, 'w');
  await campaignFile.truncate(MAX_VERIFIER_ARTIFACT_BYTES + 1);
  await campaignFile.close();
  await assert.rejects(reviewCampaignStatus(review.directory), /byte verification limit/u);

  await writeCanonicalJson(campaignPath, review.campaign);
  const packetPath = path.join(review.directory, 'packets', `${review.campaign.packetIds[0]}.json`);
  const packetFile = await open(packetPath, 'w');
  await packetFile.truncate(MAX_REVIEW_PACKET_BYTES + 1);
