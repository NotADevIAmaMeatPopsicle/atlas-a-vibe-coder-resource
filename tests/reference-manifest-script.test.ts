...
test('reference verifier bounds manifest and evidence file bytes before buffering', async (context) => {
  ...
  const encoded = await run(process.execPath, [
    '--input-type=module',
    '--eval',
    `import { terminalSafeText } from ${JSON.stringify(supportUrl)}; process.stdout.write(terminalSafeText('unsafe\\u001b[31m\\u202Ename'));`
  ]);
  assert.equal(encoded, 'unsafe\\u001B[31m\\u202Ename');

  const hostileManifest = path.join(root, 'missing\u202Emanifest.json');
  const hostileError = await runFailure(process.execPath, [
    verifier,
    '--reference', referenceRoot,
    '--manifest', hostileManifest
  ]);
  assert(!hostileError.includes('\u202E'));
  assert.match(hostileError, /\\u202E/u);

  const multilineManifest = path.join(root, 'missing\rFORGED\nmanifest.json');
  const multilineError = await runFailure(process.execPath, [
    verifier,
    '--reference', referenceRoot,
    '--manifest', multilineManifest
  ]);
  assert(!multilineError.includes('\r'));
  assert.equal((multilineError.match(/\n/gu) ?? []).length, 1);
  assert.match(multilineError, /\\u000D.*\\u000A/u);
});
