...
test('verification preserves the immutable triage-report v1.2.0 projection', async (context) => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'atlas-triage-v1-2-compatibility-'));
  context.after(async () => rm(root, { recursive: true, force: true }));
  const current = await createCurrentRun(root, 'triage-v1-2-source');
  const priorTriage = path.join(root, 'triage-v1-2-run');
  await cp(current, priorTriage, { recursive: true });
  const [run, health] = await Promise.all([
    readJson<RunRecord>(path.join(priorTriage, 'run.json')),
    readJson<AnalysisHealthRecord>(path.join(priorTriage, 'analysis-health.json'))
  ]);
  run.analyses = run.analyses.map((analysis) =>
    analysis === TRIAGE_REPORT_ANALYSIS_MARKER ? TRIAGE_REPORT_PRIOR_ANALYSIS_MARKER : analysis
  ).sort(compareCanonicalText);
  run.runId = runIdentity({
    snapshotId: run.snapshotId,
    targetId: run.targetId,
  ...
  health.runId = run.runId;
  await writeCanonicalJson(path.join(priorTriage, 'run.json'), run);
  await writeCanonicalJson(path.join(priorTriage, 'analysis-health.json'), health);
  await resealCurrentRun(priorTriage);

  const verified = await verifyAndLoadRunDirectory(priorTriage);
  assert(verified.artifacts.run.analyses.includes(TRIAGE_REPORT_PRIOR_ANALYSIS_MARKER));
  assert.match(verified.artifacts.triageReport ?? '', /Producer: `atlas\/triage-report@1\.2\.0`/u);
});
