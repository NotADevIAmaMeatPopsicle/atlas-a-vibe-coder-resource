...
import {
  loadVerifiedReviewCampaign,
  MAX_REVIEW_CAMPAIGN_BYTES,
...
async function campaignContext(campaignDirectoryValue: string, staleOnFailure: boolean): Promise<CampaignContext> {
  ...
  try {
    const verified = await loadVerifiedReviewCampaign(requested, {
      maxPackets: CAMPAIGN_PACKET_LIMIT,
      maxPacketBytes: MAX_REVIEW_PACKET_BYTES,
      maxTotalBytes: MAX_REVIEW_CAMPAIGN_BYTES
    });
    const binding = await attemptTargetBinding(verified.workspacePath, verified.campaign.runId);
    if (isInside(binding.targetRoot, verified.workspacePath)) {
      throw new AtlasError('WORKSPACE_INSIDE_TARGET', 'Review execution workspace must be outside the scanned target repository.');
    }
    return {
      campaignDirectory: verified.campaignDirectory,
