...
import {
  ...
  TRIAGE_REPORT_PRIOR_ANALYSIS_MARKER,
  TRIAGE_REPORT_PRIOR_VERSION,
  TRIAGE_REPORT_VERSION
} from './artifact-contract.js';
import { compareCanonicalText, terminalSafeDisplayText } from './util/canonical.js';
...
function inlineCode(value: string, encodeControls = false): string {
  const normalized = (encodeControls ? terminalSafeDisplayText(value) : value).replace(/\s+/gu, ' ').trim();
...
function prose(value: string, encodeControls = false): string {
  return (encodeControls ? terminalSafeDisplayText(value) : value).replace(/\s+/gu, ' ').trim()
    .replace(/&/gu, '&amp;')
    .replace(/</gu, '&lt;')
    .replace(/>/gu, '&gt;')
    .replace(/[\\`*_[\]{}#+!|]/gu, '\\$&');
...
function anchorText(finding: FindingRecord, encodeControls = false): string {
  const anchor = primaryAnchor(finding);
  if (!anchor.path) return 'not source-located';
  return inlineCode(`${anchor.path}${anchor.line ? `:${anchor.line}${anchor.column ? `:${anchor.column}` : ''}` : ''}`, encodeControls);
}
...
function affectedSurface(finding: FindingRecord, encodeControls = false): string {
  ...
  return shown.length
    ? `${scope} scope via ${shown.map((value) => inlineCode(value, encodeControls)).join(', ')}${suffix}`
    : `${scope} scope; ${prose(context.summary, encodeControls)}`;
}
...
export function renderTriageReportV1_0_0(
  ...
  for (const [index, finding] of orderedFindings.entries()) {
    ...
    lines.push(
      ...
      `- Mechanism: ${finding.signals.map((value) => inlineCode(value)).join(', ')}`,
      `- Uncertainty: confidence ${finding.confidence}; ${context?.limitations.map((value) => prose(value)).join(' ') || 'no additional limitation recorded'}`,
      '',
...
function mappingContextLines(finding: FindingRecord, encodeControls = false): string[] {
  ...
  return [
    ...
    ...contexts.map((context, index) => {
      const dockerDetails = [
        ...(context.buildContext ? [`build ${inlineCode(context.buildContext, encodeControls)}`] : []),
        ...(context.dockerfile ? [`Dockerfile ${inlineCode(context.dockerfile, encodeControls)}`] : []),
        ...(context.workingDirectory ? [`working directory ${inlineCode(context.workingDirectory, encodeControls)}`] : [])
      ];
      return `  - ${index + 1}. ${inlineCode(context.composePath, encodeControls)} · service ${inlineCode(context.service, encodeControls)} · ` +
        `${inlineCode(context.sourceKind, encodeControls)} · ${inlineCode(context.hostRoot, encodeControls)} → ${inlineCode(context.containerRoot, encodeControls)}` +
        `${dockerDetails.length ? ` · ${dockerDetails.join(' · ')}` : ''}`;
    })
  ];
}

function severityCalibrationLines(finding: FindingRecord, encodeControls = false): string[] {
  const calibration = finding.severityCalibration;
  if (!calibration) return [];
  return [
    `- Severity calibration: detector ${inlineCode(calibration.detectorSeverity, encodeControls)} → reported ${inlineCode(finding.severity, encodeControls)} ` +
      `(ceiling ${inlineCode(calibration.ceiling, encodeControls)}; basis ${inlineCode(calibration.basis, encodeControls)}; ` +
      `runtime reachability ${inlineCode(calibration.runtimeReachability, encodeControls)})`,
    `- Severity rationale: ${prose(calibration.rationale, encodeControls)}`
  ];
}

function findingPresentation(finding: FindingRecord, encodeControls = false): FindingPresentation {
  const context = finding.impactContext;
  const mechanism = finding.mechanism
    ? [
        `- Mechanism: ${inlineCode(finding.mechanism, encodeControls)}`,
        `- Signals: ${finding.signals.map((value) => inlineCode(value, encodeControls)).join(', ')}`
      ]
    : [`- Mechanism: ${finding.signals.map((value) => inlineCode(value, encodeControls)).join(', ')}`];
  return {
    heading: `${finding.severity.toUpperCase()} · ${prose(finding.title, encodeControls)}`,
    details: [
      `- Rule: ${inlineCode(finding.ruleId, encodeControls)}`,
      `- Anchor: ${anchorText(finding, encodeControls)}`,
      `- Instances: ${finding.instanceCount ?? 1}`,
      ...severityCalibrationLines(finding, encodeControls),
      `- What breaks: ${prose(finding.description, encodeControls)}`,
      `- Who/where: ${affectedSurface(finding, encodeControls)}`,
      `- When: ${activationCondition(finding)}`,
      ...mechanism,
      ...mappingContextLines(finding, encodeControls),
      `- Uncertainty: confidence ${finding.confidence}; ${context?.limitations.map((value) => prose(value, encodeControls)).join(' ') || 'no additional limitation recorded'}`
    ],
    nextValidation: `Next validation: ${prose(finding.nextValidation, encodeControls)}`
  };
}
...
function dispositionLines(diagnostics: readonly DiagnosticRecord[], encodeControls = false): string[] {
  ...
  return [
    ...
    ...applied.map((diagnostic) => {
      ...
      if (!disposition) return `- ${prose(diagnostic.message, encodeControls)}`;
      const review = disposition.reviewId ? inlineCode(disposition.reviewId, encodeControls) : 'unknown review';
      const subject = disposition.title
        ? prose(disposition.title, encodeControls)
        : disposition.findingId ? inlineCode(disposition.findingId, encodeControls) : 'suppressed finding';
      const decision = disposition.disposition ? inlineCode(disposition.disposition, encodeControls) : 'disposition applied';
      const attribution = [disposition.reviewer, disposition.date]
        .filter((value): value is string => Boolean(value))
        .map((value) => prose(value, encodeControls))
        .join(' · ');
      const rationale = disposition.evidence?.length
        ? ` Evidence: ${disposition.evidence.map((value) => prose(value, encodeControls)).join('; ')}`
        : '';
...
function renderActionabilityTriageReport(
  run: RunRecord,
  findings: readonly FindingRecord[],
  diagnostics: readonly DiagnosticRecord[],
  version: string,
  encodeControls: boolean
): string {
  const orderedFindings = [...findings].sort(compareFindingReviewPriority);
  const presentedFindings = orderedFindings.map((finding) => ({
    finding,
    presentation: findingPresentation(finding, encodeControls)
  }));
  assertDistinctFindingPresentations(presentedFindings);
  const diagnosticSummaries = summarizeDiagnostics(diagnostics);
  ...
  const lines = [
    '# Atlas triage report',
    '',
    `Run: ${inlineCode(run.runId, encodeControls)}`,
    `Snapshot: ${inlineCode(run.snapshotId, encodeControls)}`,
    `Producer: ${inlineCode(`atlas/triage-report@${version}`, encodeControls)}`,
    '',
    '## Summary',
    '',
    `- Headline findings: ${findings.length}`,
    ...
    '',
    ...dispositionLines(diagnostics, encodeControls),
    '## Findings',
    ''
  ];
  if (!orderedFindings.length) lines.push('No surfaced findings.', '');
  for (const [index, { finding, presentation }] of presentedFindings.entries()) {
    ...
    lines.push(
      `### ${index + 1}. ${presentation.heading}`,
      '',
      `Contradiction: ${prose(finding.description, encodeControls)}`,
      '',
      `Action: ${prose(finding.nextValidation, encodeControls)}`,
      '',
      `Falsifier: ${prose(finding.refutationCondition ?? 'A reviewer demonstrates that the cited static relationship is not part of the effective target behavior.', encodeControls)}`,
      '',
      `- Review ID: ${inlineCode(reviewId, encodeControls)}`,
      `- Actionability: ${priorityBand ? prose(findingReviewPriorityBandLabel(priorityBand), encodeControls) : 'legacy priority not recorded'}`,
      '',
      '<details>',
  ...
  lines.push(
    '## Diagnostics',
    '',
    `Per-instance diagnostic records are retained only in ${inlineCode('diagnostics.jsonl', encodeControls)}.`,
    ''
  );
  if (!diagnosticSummaries.length) lines.push('No diagnostics.', '');
  for (const summary of diagnosticSummaries) {
    lines.push(
      `- ${inlineCode(summary.code, encodeControls)}: ${summary.total} total (error ${summary.error}, warning ${summary.warning}, info ${summary.info})`
    );
  }
  if (diagnosticSummaries.length) lines.push('');
  return `${lines.join('\n')}\n`;
...
/** Preserve the exact v1.2.0 projection for immutable historical runs. */
export function renderTriageReportV1_2_0(
  run: RunRecord,
  findings: readonly FindingRecord[],
  diagnostics: readonly DiagnosticRecord[]
): string {
  return renderActionabilityTriageReport(run, findings, diagnostics, TRIAGE_REPORT_PRIOR_VERSION, false);
}
...
export function renderTriageReport(
  ...
  return renderActionabilityTriageReport(run, findings, diagnostics, TRIAGE_REPORT_VERSION, true);
}
...
export function renderTriageReportForMarker(
  ...
  if (marker === TRIAGE_REPORT_PRIOR_ANALYSIS_MARKER) {
    return renderTriageReportV1_2_0(run, findings, diagnostics);
  }
