...
export async function readBoundedRegularFile(
  ...
  if (options.budget && before.size > BigInt(options.budget.remainingBytes)) {
    throw new AtlasError(options.resourceCode, `${options.label} exceeds the remaining aggregate verification budget.`);
  }
  ...
  try {
    ...
    if (options.budget) options.budget.remainingBytes -= size;
    return content;
  } finally {
    await handle.close();
  }
