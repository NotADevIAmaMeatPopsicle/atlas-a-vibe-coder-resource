...
const MAX_REVIEW_CAMPAIGN_ROOT_ENTRIES = 3;
const MAX_REVIEW_CAMPAIGN_PACKETS = 25_000;
const MAX_REVIEW_ATTEMPT_RECEIPTS = 4_096;
const MAX_REVIEW_ATTEMPT_BYTES = 256 * 1024;
// A 500-file packet can exceed the generic artifact ceiling when valid long
// paths require JSON escaping. Keep this aligned with the producer contract.
export const MAX_REVIEW_PACKET_BYTES = 64 * 1024 * 1024;
export const MAX_REVIEW_CAMPAIGN_BYTES = 1024 * 1024 * 1024;

interface ReviewReadBudget {
  remainingBytes: number;
}

export interface VerifiedReviewCampaign {
...
interface ReviewCampaignReadLimits {
  maxPackets?: number;
  maxPacketBytes?: number;
  maxTotalBytes?: number;
}

async function readReviewJson<T>(
  filePath: string,
  label: string,
  invalidCode: string,
  maxBytes: number,
  ...
  return readBoundedJsonFile<T>(filePath, {
    maxBytes,
    maxDepth: MAX_VERIFIER_NESTING_DEPTH,
    resourceCode: 'REVIEW_RESOURCE_LIMIT',
    invalidCode,
    label,
    budget
  });
...
async function reviewAttemptBinding(workspacePath: string, runId: string): Promise<ReviewAttemptBinding> {
  ...
  try {
    ...
    entries = await readBoundedDirectoryEntries(attemptsDirectory, {
      maxEntries: MAX_REVIEW_ATTEMPT_RECEIPTS,
      resourceCode: 'REVIEW_RESOURCE_LIMIT',
      label: 'Review workspace attempt ledger'
    });
  } catch (error) {
  ...
  const receiptBudget: ReviewReadBudget = { remainingBytes: MAX_VERIFIER_TOTAL_BYTES };
  for (const entry of entries) {
    ...
    const receipt = await readReviewJson<ExecutionRecord>(
      receiptPath,
      `Attempt receipt ${entry.name}`,
      'REVIEW_ATTEMPT_MISMATCH',
      MAX_REVIEW_ATTEMPT_BYTES,
      receiptBudget
    );
...
export async function createReviewCampaign(options: {
  ...
  for (const [index, packet] of packets.entries()) {
    await assertSchema('review-packet', packet, `Review packet ${index + 1}`);
  }
  assertPortableDataSafe({ campaign, packets }, 'Review campaign');
  if (await exists(directory)) {
    const existing = await readReviewJson<ReviewCampaign>(
      path.join(directory, 'campaign.json'),
      'Existing review campaign',
      'REVIEW_CAMPAIGN_MISMATCH',
      MAX_VERIFIER_ARTIFACT_BYTES,
      { remainingBytes: MAX_VERIFIER_TOTAL_BYTES }
    );
...
export async function loadVerifiedReviewCampaign(
  campaignDirectoryValue: string,
  limits: ReviewCampaignReadLimits = {}
): Promise<VerifiedReviewCampaign> {
  const maxPackets = limits.maxPackets ?? MAX_REVIEW_CAMPAIGN_PACKETS;
  const maxPacketBytes = limits.maxPacketBytes ?? MAX_REVIEW_PACKET_BYTES;
  const maxTotalBytes = limits.maxTotalBytes ?? MAX_REVIEW_CAMPAIGN_BYTES;
  ...
  try {
    ...
    campaignEntries = await readBoundedDirectoryEntries(requestedCampaignDirectory, {
      maxEntries: MAX_REVIEW_CAMPAIGN_ROOT_ENTRIES,
      resourceCode: 'REVIEW_RESOURCE_LIMIT',
      label: 'Review campaign directory'
    });
  } catch (error) {
    if (error instanceof AtlasError) throw error;
  ...
  const campaignBudget: ReviewReadBudget = { remainingBytes: maxTotalBytes };
  const campaign = await readReviewJson<ReviewCampaign>(
  ...
  await assertSchema('review-campaign', campaign, 'Review campaign');
  if (campaign.packetIds.length > maxPackets) {
    throw new AtlasError('REVIEW_RESOURCE_LIMIT', `Review campaign exceeds the ${maxPackets}-packet verification limit.`);
  }
  ...
  try {
    ...
    packetEntries = await readBoundedDirectoryEntries(packetsDirectory, {
      maxEntries: maxPackets + 1,
      resourceCode: 'REVIEW_RESOURCE_LIMIT',
      label: 'Review campaign packet directory'
    });
  } catch (error) {
    if (error instanceof AtlasError) throw error;
  ...
  const packetRecords: ReviewPacket[] = [];
  for (const [index, packetId] of campaign.packetIds.entries()) {
    const packetPath = path.join(packetsDirectory, `${packetId}.json`);
    const packet = await readReviewJson<ReviewPacket>(
      packetPath,
    ...
    packetRecords.push(packet);
  }
  return { campaignDirectory, workspacePath, campaign, packets: packetRecords };
}

export async function reviewCampaignStatus(campaignDirectoryValue: string): Promise<unknown> {
  const verified = await loadVerifiedReviewCampaign(campaignDirectoryValue);
  const packets = verified.packets.map((packet) => ({
    packetId: packet.packetId,
    state: packet.state,
    fileCount: packet.files.length,
    estimatedInputTokens: packet.estimatedInputTokens
  }));
  return { campaign: verified.campaign, packets };
}
