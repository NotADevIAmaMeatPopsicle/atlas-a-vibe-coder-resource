...
## Public Git repository contents

The public source boundary is exactly the set reported by `git ls-files` in a
clean checkout. It contains:
...
- public architecture, development, security, and data-policy documentation
  under `docs/`; and
...
Apache-2.0 license and author metadata apply to this source repository, but an
npm release requires a separate owner decision. Repository, homepage, and issue
metadata point to the canonical GitHub repository. Forks and local branches may
point elsewhere, so verify the destination before any push or release.
