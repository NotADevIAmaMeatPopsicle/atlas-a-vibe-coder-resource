...
async function main() {
  ...
  process.stdout.write(`${terminalSafeText(JSON.stringify({ outputPath, fileCount: manifest.fileCount, totalBytes: manifest.totalBytes, aggregateSha256: referenceDigest }))}\n`);
}

main().catch((error) => {
  process.stderr.write(`${terminalSafeText(error instanceof Error ? error.message : String(error))}\n`);
  process.exitCode = 1;
});
